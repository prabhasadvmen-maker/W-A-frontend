...
import { LayoutDashboard, Users, Shield, LogOut, Settings, ChevronRight, User } from 'lucide-react'
...
const links = [
  ...
  { to: '/admin/clients', label: 'CLIENT ACCOUNTS', icon: Shield },
      ...
      >
        <div className={`flex h-20 items-center border-b border-emerald-900/60 ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-5'}`}>
          <div className="h-10 w-10 rounded-full bg-[#25D366] flex items-center justify-center text-[#0B2818] shadow-md shrink-0 font-black">
            <Shield className="h-6 w-6 text-[#0B2818]" />
          </div>
          {!isCollapsed && (
            <div className="min-w-0">
              ...
              <p className="text-[11px] font-bold text-emerald-400">Reseller Portal</p>
            </div>
          )}
        </div>
